<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    ...
    <div class="flex justify-end mb-4 gap-2">
      <a href="{{ route('printAllSuratKerjas') }}" target="_blank"
        ...
        Cetak Data Surat Kerja
      </a>
      <a href="{{ route('suratkerjas.create') }}"
        ...
        x-show="('{{ auth()->user()->hasRole('bawaslu-kabupaten-kota') }}')">
        ...
        <table class="w-full whitespace-no-wrap">
          <thead>
            <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase bg-gray-50 border-b">
              ...
              <th class="px-4 py-3">Assign by</th>
          ...
          <tbody class="bg-white divide-y">
            @forelse ($suratKerjas as $surat)
              <tr class="text-gray-700">
                <td class="px-4 py-3 text-sm">
                  {{ $surat->nomor_surat_kerja }}
                </td>
                <td class="px-4 py-3 text-sm">
                  {{ $surat->assignBy->name }}
                </td>
                ...
                <td class="px-4 py-3 text-sm">
                  {{ $surat->pelanggaran_count }}
                </td>
                <td class="px-4 py-3 text-sm flex space-x-2">
                  <a href="{{ route('suratkerjas.show', $surat->id) }}"
                  ...
                  <a href="{{ route('printAllSuratKerjasById', $surat->id) }}" target="_blank"
                  ...
                  <a href="{{ route('suratkerjas.edit', $surat->id) }}"
                      ...
                      <div x-show="openDelete" x-transition:enter="transition ease-out duration-150"
                        ...
                        <div class="mt-4 mb-6">
                          ...
                          <p class="mb-2 text-lg font-semibold text-gray-700">
                            Hapus Data Surat Kerja
                          </p>
                          <!-- Modal description -->
                          <p class="text-sm text-gray-700">
                            Apakah Anda yakin ingin menghapus data Surat Kerja <span
                              class="font-semibold">{{ $surat->nomor_surat_kerja }}</span>?
                          </p>
                        </div>
                        <footer
                          ...
                          <form action="{{ route('suratkerjas.destroy', $surat->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            @empty
              <tr>
                <td colspan="4" class="px-4 py-3 text-sm text-center text-gray-700">
                  Data Surat Kerja tidak ditemukan.
                </td>
              </tr>
            @endforelse
          </tbody>
