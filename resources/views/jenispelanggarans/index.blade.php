<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    ...
    <div class="flex justify-end mb-4 gap-2">
      <a href="{{ route('printAllJenisPelanggarans') }}" target="_blank"
          ...
          <tbody class="bg-white divide-y">
            @forelse ($jenispelanggarans as $jenis)
              <tr class="text-gray-700">
                ...
                <td class="px-4 py-3 text-sm">
                  {{ $jenis->pelanggaran_count }}
                </td>
                <td class="px-4 py-3 text-sm flex space-x-2">
                  <a href="{{ route('jenispelanggarans.show', $jenis->id) }}"
                  ...
                  <a href="{{ route('printAllJenisPelanggaransById', $jenis->id) }}" target="_blank"
                  ...
                  <a href="{{ route('jenispelanggarans.pelanggarans', $jenis->id) }}"
                    class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                      viewBox="0 0 24 24">
                  ...
                  <a href="{{ route('jenispelanggarans.edit', $jenis->id) }}"
                      ...
                      <div x-show="openDelete" x-transition:enter="transition ease-out duration-150"
                        ...
                        <div class="mt-4 mb-6">
                          ...
                          <p class="mb-2 text-lg font-semibold text-gray-700">
                            Hapus Data Jenis Pelanggaran
                          </p>
                          <!-- Modal description -->
                          <p class="text-sm text-gray-700">
                            Apakah Anda yakin ingin menghapus data jenis pelanggaran <span
                              class="font-semibold">{{ $jenis->jenis_pelanggaran }}</span>?
                          </p>
                        </div>
                        <footer
                          ...
                          <form action="{{ route('jenispelanggarans.destroy', $jenis->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            @empty
              <tr>
                <td colspan="2" class="px-4 py-3 text-sm text-center text-gray-700">
                  Data Partai Politik tidak ditemukan.
                </td>
              </tr>
            @endforelse
          </tbody>
